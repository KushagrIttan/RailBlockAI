  ...
  return (
    <div className="panel-surface overflow-hidden">
      ...
      <div className="flex items-center gap-2 border-b border-border px-4 py-3.5">
        ...
        {pending.length > 0 && (
          <span className="ml-auto rounded-full bg-tint-warning px-2 py-0.5 text-[11px] font-semibold text-ink-warning">
      ...
      <div className="divide-y divide-border">
        {/* Onboarding tip */}
        {showBanner && (
          <div className="relative bg-tint-primary px-4 py-3">
            <button
          ...
          return (
            ...
            >
              <div className="flex items-start gap-2.5">
                <AlertTriangle
                  className={`mt-0.5 size-4 shrink-0 ${critical ? "text-ink-destructive" : "text-ink-warning"}`}
                />
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase ${
                        critical
                          ? "bg-tint-destructive text-ink-destructive"
                          : "bg-tint-warning text-ink-warning"
                      }`}
                    >
                      {conflict.severity}
                    </span>
        ...
        {/* Cleared items */}
        {cleared.length > 0 && (
          <div className="border-t border-border bg-muted px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
            Resolved by engine
          </div>
        )}
        {cleared.map((conflict) => {
          ...
          return (
            ...
            >
              <div className="flex items-start gap-2.5">
                <CheckCircle2 className="mt-0.5 size-4 shrink-0 text-ink-success" />
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-semibold text-muted-foreground">
                      Maintenance decision
                    </span>
                    <span className="rounded bg-tint-success px-1.5 py-0.5 text-[10px] font-semibold text-ink-success border border-success">
                      resolved
                    </span>
                  </div>
                  {rec && (
